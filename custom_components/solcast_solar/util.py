...
def cubic_interp(x0: list[Any], x: list[Any], y: list[Any]) -> list[Any]:
    ...
    def clip(lst: list[float], min_val: float, max_val: float, in_place: bool = False) -> list[float]:  # numpy-like clip
        if not in_place:
            lst = lst[:]
    ...
    def search_sorted(list_to_insert: list[float], insert_into: list[float]) -> list[float]:  # numpy-like search_sorted
        def float_search_sorted(float_to_insert: float, insert_into: list[float]) -> int:
    ...
    size: int = len(x)
    x_diff: list[int] = diff(x)
    y_diff: list[float] = diff(y)

    li: list[float] = [0] * size
    li_1: list[float] = [0] * (size - 1)
    z: list[float] = [0] * (size)
    ...
    xi1: list[float] = [x[num] for num in index]
    xi0: list[float] = [x[num - 1] for num in index]
    yi1: list[float] = [y[num] for num in index]
    yi0: list[float] = [y[num - 1] for num in index]
    zi1: list[float] = [z[num] for num in index]
    zi0: list[float] = [z[num - 1] for num in index]
    hi1 = list(map(subtract, xi1, xi0))

    f0: list[float] = [0] * len(hi1)
