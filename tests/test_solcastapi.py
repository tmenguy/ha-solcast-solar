...
async def test_build_splines(hass: HomeAssistant) -> None:
    ...
    # Forecast fetch with past actuals, one API key, two sites.
    global MOCK_SOLCAST1  # noqa: PLW0603
    solcast = await __get_solcast(hass, MOCK_ENTRY1)
    assert solcast._data == FRESH_DATA
    assert solcast._data_undampened == FRESH_DATA
    assert await solcast.get_forecast_update(do_past=True, force=False) == ""
...
async def test_get_total_energy_forecast(hass: HomeAssistant) -> None:
    ...
    for day in range(7):
        if not build:
            assert solcast.get_total_energy_forecast_day(day) == expect["pv_estimate"]
            for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
                assert solcast.get_total_energy_forecast_day(day, forecast_confidence=estimate) == expect[estimate]
        else:
            if day > 0:
                break
            expect = {}
            for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
                expect[estimate] = solcast.get_total_energy_forecast_day(day, forecast_confidence=estimate)
            _LOGGER.info(expect)
...
async def test_get_peaks(hass: HomeAssistant) -> None:
    ...
    if build:
        expect = {"pv_estimate": {}, "pv_estimate10": {}, "pv_estimate90": {}}
    for day in range(7):
        if not build:
            assert solcast.get_peak_time_day(day).replace(year=2024, month=1, day=1) == dt.fromisoformat(expect["pv_estimate"]["peak_time"])
            assert solcast.get_peak_power_day(day) == expect["pv_estimate"]["peak_power"]
        for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
            if not build:
                assert solcast.get_peak_time_day(day, forecast_confidence=estimate).replace(year=2024, month=1, day=1) == dt.fromisoformat(
                    expect[estimate]["peak_time"]
                )
                assert solcast.get_peak_power_day(day, forecast_confidence=estimate) == expect[estimate]["peak_power"]
            else:
                if day > 0:
                    break
                expect[estimate] = {
...
async def test_get_power_n_minutes(hass: HomeAssistant) -> None:
    ...
    if not build:
        for minute in range(0, 61, 30):
            assert solcast.get_power_n_minutes(minute) == expect["pv_estimate"][minute]
    else:
        expect = {}
        expect_site = {}
    for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
        ...
        for minute in range(0, 61, 30):
            if not build:
                assert solcast.get_power_n_minutes(minute, forecast_confidence=estimate) == expect[estimate][minute]
            else:
                expect[estimate][minute] = solcast.get_power_n_minutes(minute, forecast_confidence=estimate)
    for site in solcast.sites:
        if build:
            expect_site[site["resource_id"]] = {"pv_estimate": {}, "pv_estimate10": {}, "pv_estimate90": {}}
        for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
            for minute in range(0, 61, 30):
                if not build:
                    assert (
                        solcast.get_power_n_minutes(minute, forecast_confidence=estimate, site=site["resource_id"])
                        == expect_site[site["resource_id"]][estimate][minute]
                    )
                else:
                    expect_site[site["resource_id"]][estimate][minute] = solcast.get_power_n_minutes(
...
async def test_get_forecast_n_hour(hass: HomeAssistant) -> None:
    ...
    build = False
    expect = {"pv_estimate": {0: 7200, 1: 6732}, "pv_estimate10": {0: 6000, 1: 5610}, "pv_estimate90": {0: 8000, 1: 7480}}
    expect_site = {
        "1111-1111-1111-1111": {
    ...
    if not build:
        for hour in range(2):
            assert solcast.get_forecast_n_hour(hour) == expect["pv_estimate"][hour]
    else:
        expect = {}
        expect_site = {}
    for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
        ...
        for hour in range(2):
            if not build:
                assert solcast.get_forecast_n_hour(hour, forecast_confidence=estimate) == expect[estimate][hour]
            else:
                expect[estimate][hour] = solcast.get_forecast_n_hour(hour, forecast_confidence=estimate)
    for site in solcast.sites:
        if build:
            expect_site[site["resource_id"]] = {"pv_estimate": {}, "pv_estimate10": {}, "pv_estimate90": {}}
        for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
            for hour in range(2):
                if not build:
                    assert (
                        ...
                        == expect_site[site["resource_id"]][estimate][hour]
                    )
                else:
                    expect_site[site["resource_id"]][estimate][hour] = solcast.get_forecast_n_hour(
...
async def test_get_forecast_custom_hours(hass: HomeAssistant) -> None:
    ...
    build = False
    expect = {"pv_estimate": 13748, "pv_estimate10": 11457, "pv_estimate90": 15276}
    expect_site = {
    ...
    expect_long = {"pv_estimate": 151449, "pv_estimate10": 126207, "pv_estimate90": 168276}

    if not build:
        assert solcast.get_forecast_custom_hours(CUSTOM_HOURS) == expect["pv_estimate"]
    else:
        expect = {}
        expect_site = {}
    for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
        if not build:
            assert solcast.get_forecast_custom_hours(CUSTOM_HOURS, forecast_confidence=estimate) == expect[estimate]
            assert solcast.get_forecast_custom_hours(90, forecast_confidence=estimate) == expect_long[estimate]
        else:
            expect[estimate] = solcast.get_forecast_custom_hours(CUSTOM_HOURS, forecast_confidence=estimate)
            expect_long[estimate] = solcast.get_forecast_custom_hours(90, forecast_confidence=estimate)
    for site in solcast.sites:
        if build:
            expect_site[site["resource_id"]] = {}
        for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
            if not build:
                assert (
                    solcast.get_forecast_custom_hours(CUSTOM_HOURS, forecast_confidence=estimate, site=site["resource_id"])
                    == expect_site[site["resource_id"]][estimate]
                )
            else:
                expect_site[site["resource_id"]][estimate] = solcast.get_forecast_custom_hours(
...
async def test_get_forecast_remaining_today(hass: HomeAssistant) -> None:
    ...
    build = False
    expect = {"pv_estimate": 23.6817, "pv_estimate10": 19.7348, "pv_estimate90": 26.313}
    expect_site = {
    ...
    if not build:
        assert solcast.get_forecast_remaining_today() == expect["pv_estimate"]
    else:
        expect = {}
        expect_site = {}
    for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
        if not build:
            assert solcast.get_forecast_remaining_today(forecast_confidence=estimate) == expect[estimate]
        else:
            expect[estimate] = solcast.get_forecast_remaining_today(forecast_confidence=estimate)
    for site in solcast.sites:
        if build:
            expect_site[site["resource_id"]] = {}
        for estimate in ["pv_estimate", "pv_estimate10", "pv_estimate90"]:
            if not build:
                ...
                expect_site[site["resource_id"]][estimate] = solcast.get_forecast_remaining_today(
